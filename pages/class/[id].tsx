...
export default function Home(props) {
  ...
  const [data, setData] = useState({
    ...
    createdAt: null,
    members: [],
    teacher: { name: null, profile: null },
  });
    ...
    <Layout title="classes">
      <div className="flex bg-gray-900 mx-auto justify-center py-4">
        {state === 1 ? (
          ...
          <button
            onClick={() => setState(1)}
            className="h-10 px-4 py-2 -mb-px text-sm text-center text-gray-700 bg-transparent border-b-2 border-transparent sm:text-base dark:text-white whitespace-nowrap cursor-base focus:outline-none hover:border-gray-400"
          >
            Posts
          </button>
        )}
          ...
          {!data ? (
            ...
            <div>
              <div className="max-w-5xl  mx-auto text-center overflow-hidden bg-blend-darken rounded-lg shadow-md  bg-gray-800">
                <img
                  className="object-cover w-full h-64 bg-gray-900 bg-blend-darken"
                  src={profile}
                  alt="Article"
                />
                <div
                  ...
                  className="text-3xl max-w-5xl  font-semibold text-center text-white "
                >
                  {data.name}
                </div>
                <div
                ...
                {!share ? (
                    ...
                    <div className="flex items-center self-stretch justify-center">
                      <div className="relative flex-shrink-0 px-2">
                        <img
                          src={uprofile}
                          alt=""
                          className="w-12 h-12  rounded-full  bg-coolGray-500  border-coolGray-700"
                        />
                      </div>
                    </div>
                    <span
                      className="text-gray-200  cursor-pointer hover:text-blue-600"
                      onClick={() => {
                        setShare(!share);
                      }}
                    >
                      Click here to post something to the class
                    </span>
                  </div>
                ) : (
                  <div className="flex items-center p-6  md:mx-auto space-x-4 rounded-md  bg-gray-800  text-white">
                    <div className="w-full mt-4">
                      ...
                      <button
                        className="px-4 py-2  font-medium tracking-wide text-white capitalize transition-colors duration-200 transform  rounded-md "
                        onClick={() => {
                          setShare(!share);
                        }}
                      >
              ...
              <div className="max-w-3xl space-y-4 mx-5 md:mx-auto overflow-hidden   ">
                {posts &&
                  posts.map((p) => (
                    <div
                      key={p.posts}
                      className="  p-6 my-4 margin-bg-white space-x-4 rounded-md  bg-gray-800  text-white"
                    >
          ...
          <section className="py-6 bg-coolGray-800 text-coolGray-100">
            <div className="container p-4 mx-auto space-y-16 sm:p-10">
              <div className="space-y-4">
                <img
                  alt=""
                  className="object-cover h-56 mx-auto mb-4 bg-center rounded-sm bg-coolGray-500"
                ...
                {data.members.map((p) => (
                  <div key={p} className="space-y-4">
                    ...
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-sm text-gray-400 text-coolGray-400">
                        Student
                      </p>
                    </div>
                  </div>
              ...
              <div className="max-w-3xl space-y-4 mx-5 md:mx-auto overflow-hidden   ">
                {assignments &&
                  assignments.map((p) => (
                    ...
                    >
                      <div className="flex  items-center  margin-bg-white space-x-4">
                        <div className="flex items-center self-stretch justify-center"></div>
                      ...
                      {type(p.media) != "image" && p.media && (
                        <div className="flex">
                          <a
                            className="flex hover:text-blue-600 my-3 hover:underline"
                            download="File"
                            href={p.media}
                            title=""
                          >
                      ...
                      {admin == false ? (
                        <button
                          onClick={() => router.push(`/assignment/${p._id}`)}
                          className="px-2 mt-4 py-1  font-medium tracking-wide text-white capitalize transition-colors duration-200 transform bg-blue-600 rounded-md hover:bg-blue-500 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-80"
                        >
                          Submit
                        </button>
                      ) : (
                        <button
                          onClick={() => router.push(`/submissions/${p._id}`)}
                          className="px-2 mt-4 py-1  font-medium tracking-wide text-white capitalize transition-colors duration-200 transform bg-blue-600 rounded-md hover:bg-blue-500 focus:outline-none focus:ring focus:ring-blue-300 focus:ring-opacity-80"
                        >
                          Submissions
                        </button>
